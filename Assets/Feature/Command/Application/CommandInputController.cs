...
namespace Feature.Command.Application
{
    public sealed class CommandInputController : IDisposable
    {
        ...
        {
            _input.PointerPosition.Subscribe(pos =>
            {
                _currentPointer = pos;
                _commandStateMachine.Current.OnPointerMove(pos);
            }).AddTo(_disposables);
            
            _input.ClickDown.Subscribe(_ => _commandStateMachine.Current.OnPointerDown(_currentPointer)).AddTo(_disposables);
            _input.ClickUp.Subscribe(_ => _commandStateMachine.Current.OnPointerUp(_currentPointer)).AddTo(_disposables);
            _input.ShiftDown.Subscribe(_ => _commandStateMachine.Current.SetShiftHold(true)).AddTo(_disposables);
            _input.ShiftUp.Subscribe(_ => _commandStateMachine.Current.SetShiftHold(false)).AddTo(_disposables);
            _input.CtrlDown.Subscribe(_ => _commandStateMachine.Current.SetCtrlHold(true)).AddTo(_disposables);
            _input.CtrlUp.Subscribe(_ => _commandStateMachine.Current.SetCtrlHold(false)).AddTo(_disposables);
