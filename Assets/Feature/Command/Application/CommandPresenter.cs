...
namespace Feature.Command.Application
{
    public sealed class CommandPresenter : IInitializable, IDisposable
    {
        ...
        private void KeyboardSubscribe()
        {
            ...
            _input.Select.Subscribe(_ => _commandStateMachine.SetSelect()).AddTo(_disposables);
            _input.Move.Subscribe(_ => _commandStateMachine.SetMove()).AddTo(_disposables);
            
            _commandView.Initialize();
            _commandStateMachine.SetSelect();
        }

        private void ViewSubscribe()
        {
            _commandView.GetClick(CommandType.Select).Subscribe(_ => _commandStateMachine.SetSelect()).AddTo(_disposables);
            _commandView.GetClick(CommandType.Move).Subscribe(_ => _commandStateMachine.SetMove()).AddTo(_disposables);
        }
