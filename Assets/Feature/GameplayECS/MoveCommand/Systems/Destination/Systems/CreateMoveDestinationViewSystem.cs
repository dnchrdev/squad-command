...
{
    public class CreateMoveDestinationViewSystem : ISystem
    {
        ...
        public CreateMoveDestinationViewSystem(IObjectPool<IMoveSlotView> pool, IMoveSlotViewFactory moveSlotViewFactory)
        {
            _pool = pool;
        ...
        private async UniTask ProcessPendingViewsAsync()
        {
            ...
            while (_pendingSlots.Count > 0)
            {
                ...
                var view = await _moveSlotViewFactory.CreateAsync(assetPath, position, rotation);
                view.Hide();
